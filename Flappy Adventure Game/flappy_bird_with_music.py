...
# Set up audio system
pygame.mixer.set_reserved(4)  # Reserve channels for our sounds
pygame.mixer.music.set_volume(0.7)  # Set music volume
...
pygame.display.set_caption("Flappy Bird - With Music")
clock = pygame.time.Clock()

# Sound effects (we'll create simple tones)
def create_sound_frequency(frequency, duration=100):
    ...
    try:
        ...
        wave = np.sin(2 * np.pi * frequency * t) * 0.5  # Increased volume
...
try:
    flap_sound = create_sound_frequency(800, 150)  # High pitch flap sound
    score_sound = create_sound_frequency(1200, 200)  # Higher pitch for scoring
    crash_sound = create_sound_frequency(200, 500)  # Low pitch for crash
    theme_change_sound = create_sound_frequency(600, 300)  # Medium pitch for theme change
    
    # Test sound creation
    print("🎵 Sound effects created successfully!")
    if flap_sound:
        print("✅ Flap sound ready")
    if score_sound:
        print("✅ Score sound ready")
    if crash_sound:
        print("✅ Crash sound ready")
    if theme_change_sound:
        print("✅ Theme change sound ready")
...
def create_background_music():
    ...
    try:
        ...
        # Create a simple melody with more notes
        melody_notes = [262, 330, 392, 523, 392, 330, 262, 330]  # C, E, G, C, G, E, C, E
        melody_duration = duration / len(melody_notes)
        melody_samples = int(sample_rate * melody_duration)
        
        full_melody = np.array([])
        for note in melody_notes:
            t = np.linspace(0, melody_duration, melody_samples)
            wave = np.sin(2 * np.pi * note * t) * 0.3  # Increased volume
            full_melody = np.concatenate([full_melody, wave])
        
        # Ensure correct length
        if len(full_melody) < samples:
            full_melody = np.pad(full_melody, (0, samples - len(full_melody)))
        else:
            full_melody = full_melody[:samples]
        
        wave = (full_melody * 32767).astype(np.int16)
...
class Game:
    ...
    def draw(self):
        ...
        if not self.game_started and not self.game_over:
            ...
            instruction4 = self.small_font.render("🎵 Music and sound effects included!", True, WHITE)
