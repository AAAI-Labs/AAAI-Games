...
MIN_OBSTACLE_SPACING = 250  # Reduced for more frequent obstacles
...
class Level:
    def __init__(self, level_num, name, description, color):
        ...
        # Level-specific settings - MUCH HARDER
        if level_num == 1:
            self.obstacle_speed = 5  # Increased from 3
            self.obstacle_spawn_rate = 0.025  # Increased from 0.01
            self.obstacle_types = ["spike", "block", "flying_spike"]
            self.platform_types = ["normal", "small"]
            self.platform_spawn_rate = 0.02
            self.required_score = 25  # Increased from 10
            self.background_color = BLUE
        elif level_num == 2:
            self.obstacle_speed = 7  # Increased from 4
            self.obstacle_spawn_rate = 0.035  # Increased from 0.015
            ...
            self.required_score = 45  # Increased from 20
            self.background_color = PURPLE
        elif level_num == 3:
            self.obstacle_speed = 9  # Increased from 5
            self.obstacle_spawn_rate = 0.045  # Increased from 0.02
            ...
            self.required_score = 70  # Increased from 35
            self.background_color = RED
        elif level_num == 4:
            self.obstacle_speed = 11  # Increased from 6
            self.obstacle_spawn_rate = 0.055  # Increased from 0.025
            self.obstacle_types = ["spike", "block", "flying_spike", "double_spike", "moving_spike", "laser"]
            self.platform_types = ["normal", "small", "moving", "disappearing", "bouncy"]
            self.platform_spawn_rate = 0.05
            self.required_score = 100  # Increased from 50
            self.background_color = ORANGE
        elif level_num == 5:
            self.obstacle_speed = 13  # Increased from 7
            self.obstacle_spawn_rate = 0.065  # Increased from 0.03
            self.obstacle_types = ["spike", "block", "flying_spike", "double_spike", "moving_spike", "laser", "triple_spike"]
            self.platform_types = ["normal", "small", "moving", "disappearing", "bouncy", "teleport"]
            self.platform_spawn_rate = 0.06
            self.required_score = 150  # Increased from 75
    ...
    def jump(self):
        if self.is_on_ground or self.is_on_platform:
            ...
            if self.current_platform and self.current_platform.platform_type == "bouncy":
                self.velocity_y = JUMP_FORCE * 1.5  # Higher jump
            else:
...
class Platform:
    ...
    def update(self):
        ...
        if self.platform_type == "moving":
            self.y = self.original_y + math.sin(self.movement_timer * 0.1) * 40
            
        # Disappearing platform logic
        elif self.platform_type == "disappearing":
            self.disappear_timer += 1
            if self.disappear_timer > 120:  # Disappear after 2 seconds
                self.visible = False
                
        # Teleport platform logic
        elif self.platform_type == "teleport":
            self.teleport_timer += 1
            if self.teleport_timer > 180:  # Teleport every 3 seconds
                self.y = random.randint(GROUND_Y - 300, GROUND_Y - 100)
                self.teleport_timer = 0
    ...
    def draw(self, screen):
        ...
        if self.platform_type == "disappearing":
            # Add warning effect
            if self.disappear_timer > 90:  # Flash when about to disappear
                if self.disappear_timer % 20 < 10:
            ...
            # Add teleport effect
            if self.teleport_timer > 150:  # Flash before teleporting
                if self.teleport_timer % 15 < 7:
...
class Obstacle:
    def __init__(self, x, obstacle_type="spike", level_settings=None):
        ...
        if obstacle_type == "spike":
            ...
            self.height = 45
            self.color = PURPLE
            self.width = 60  # Wider for double spike
            ...
            self.height = 45
            self.color = PINK
            self.width = 90  # Wider for triple spike
    ...
    def draw(self, screen):
        ...
        elif self.obstacle_type == "triple_spike":
            # Draw triple spike
            points1 = [(self.x, GROUND_Y), (self.x + 15, GROUND_Y - self.height), (self.x + 30, GROUND_Y)]
            points2 = [(self.x + 30, GROUND_Y), (self.x + 45, GROUND_Y - self.height), (self.x + 60, GROUND_Y)]
            points3 = [(self.x + 60, GROUND_Y), (self.x + 75, GROUND_Y - self.height), (self.x + 90, GROUND_Y)]
            pygame.draw.polygon(screen, self.color, points1)
            pygame.draw.polygon(screen, self.color, points2)
            pygame.draw.polygon(screen, self.color, points3)
...
class Game:
    ...
    def draw_menu(self):
        ...
        subtitle = self.font.render("Ultimate Edition", True, YELLOW)
